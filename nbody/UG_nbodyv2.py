...
class Particles:
    def __init__(self, n: int, box: float):
        ...
        self.x  = rng.random((n, 2)) * box        # comoving positions
        self.p  = np.zeros((n, 2))                # canonical momenta (comoving)
        self.m  = np.ones(n) * (box**2 / n)       # equal masses
        self.box = box
    ...
    def deposit_density(self, mesh_size: int, a: float) -> tuple[np.ndarray, float]:
        ...
        # periodic images via floor division
        gx = (self.x[:, 0] / h).astype(int) % mesh_size
        gy = (self.x[:, 1] / h).astype(int) % mesh_size
        np.add.at(rho, (gx, gy), self.m)
        rho /= h**2 * a**2                     # physical density  ρ = m / (a² dA)
        return rho, rho.mean()

    # gather forces and advance momenta + positions (kick-drift-kick)
    def advance(self, acc: np.ndarray, dt: float, bg: Background):
        ...
        ## half-kick
        ## self.p += 0.5 * dt * acc * bg.a**2      # p updates in comoving coord.
        # drift (x' = p / (a m))
...
def run(mesh_size: int, n_particles: int, dt: float, n_steps: int, box: float, H0: float | None):
    ...
    for istep in range(n_steps):
        ...
        a_hist.append([istep*dt,bg.a,bg.H])
